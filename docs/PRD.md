...
1. [产品定位](#1-产品定位)（含市场规模、竞品分析、商业模式、GTM 策略）
...
9. [用户研究](#9-用户研究)
10. [风险分析](#10-风险分析)
11. [v1.1+ Backlog](#11-v11-backlog)
12. [更新日志](#12-更新日志)
...
> ⚠️ 以下数据基于公开报告估算，需 Sensor Tower / Statista 实际数据验证。

| 维度 | 规模 | 说明 |
|---|---|---|
| **TAM**（总可触达市场） | ~$8B（全球时间管理/生产力工具市场） | 包含所有时间追踪、任务管理、日历工具。2025 年全球生产力软件市场约 $50-60B，时间管理子赛道约占 15% |
| **SAM**（可服务市场） | ~$500M（中国 C 端学生/职场新人时间管理工具） | 中国在校大学生 ~4700 万 + 职场新人（工作 1-3 年）~3000 万，假设 10% 有付费意愿 × ARPU ¥50/年 |
| **SOM**（可获得市场） | ~5000 用户（Year 1 目标） | v1 仅 H5 + 一个人开发，保守估计首年获取 5000 活跃用户 |

**增长驱动因素**：
- 远程/混合办公常态化，个人时间管理需求上升
- AI 热潮带动效率工具关注度
- 学生群体对"计划失败感"的共鸣（小红书/即刻上大量相关内容）
...
| 竞品 | 类型 | 选择理由 |
|---|---|---|
| **Forest** | 专注计时（游戏化） | C 端最成功的专注计时 app，全球 1000 万+ 用户 |
| **番茄 Todo** | 番茄钟 + 任务管理 | 中国市场直接竞品，学生群体高渗透 |
| **滴答清单** | 全功能任务管理 | 中国市场标杆，功能全面 |
| **Toggl Track** | 时间记账 | 专业时间追踪工具，record-first 最接近的竞品 |
| **RescueTime** | 自动时间追踪 | 被动记录型，与 Peel 的 record-first 有交叉 |

#### 核心功能对比

| 功能维度 | Peel | Forest | 番茄 Todo | 滴答清单 | Toggl |
|---------|------|--------|----------|---------|-------|
| 正向计时 | ✅ 核心 | ❌ | ✅ | ✅ | ✅ |
| 番茄钟 | ✅ 模式 | ✅ 核心 | ✅ 核心 | ✅ | ❌ |
| Todo | ✅ 挂 record | ❌ | ✅ | ✅ 核心 | ❌ |
...
| 竞品 | 免费版 | 付费版 | 付费功能 |
|------|--------|--------|---------|
| Forest | 基础计时 | ¥25/年 | 更多树种、标签、统计 |
| 番茄 Todo | 基础番茄 | ¥98/年 | 高级统计、多设备同步 |
| 滴答清单 | 5 个清单 | ¥139/年 | 无限清单、日历、提醒 |
| Toggl | 5 用户 | $9/用户/月 | 团队功能、报表 |
| **Peel** | **全功能免费** | v1.1+ 考虑 | v1 不收费，验证产品价值优先 |

#### 用户评价分析（需实际数据验证）

| 竞品 | 好评关键词 | 差评关键词 |
|------|-----------|-----------|
| Forest | "有成就感"、"种树有趣" | "离开 app 树就死"、"对真拖延症没用" |
| 番茄 Todo | "简洁"、"番茄钟好用" | "功能太少"、"统计不详细" |
| 滴答清单 | "功能全面"、"跨平台" | "太复杂"、"学习成本高" |
| Toggl | "专业"、"报表强大" | "太工具感"、"不适合个人" |

**Peel 的机会**：竞品要么偏游戏化（Forest）、要么偏工具感（Toggl）、要么偏功能堆砌（滴答清单）。没有一个产品在做"用真实数据帮用户校准时间预期"。
...
| 模式 | 描述 | 预估 ARPU | 适用时机 |
|------|------|----------|---------|
| **Freemium** | 基础免费 + 高级功能付费（暗色模式/数据导入/翻页时钟） | ¥30-50/年 | 用户量 > 1000 |
| **一次性买断** | 付费解锁全部功能 | ¥28-48 | 用户反馈"愿意付费" |
| **订阅制** | 月付/年付 | ¥8/月 或 ¥58/年 | 功能足够多，持续提供价值 |

**推荐路径**：Freemium（基础免费 + 高级付费），参考 Forest 模式。

#### 成本结构

| 成本项 | 月成本 | 说明 |
|--------|--------|------|
| 域名 | ¥0（GitHub Pages） | v1 部署到 Vercel/Cloudflare |
| 服务器 | ¥0 | 纯前端，无后端 |
| AI API | ¥0 | v1 不接 LLM |
| 设计工具 | ¥0 | Figma 免费版 |
| **总计** | **¥0** | v1 零成本运行 |
...
| 渠道 | 策略 | 预估触达 |
|------|------|---------|
| **小红书** | "效率焦虑"相关内容 + 产品截图 | 核心渠道，学生群体集中 |
| **即刻** | 产品发布帖 + 效率工具圈 | 早期采用者 |
| **少数派** | 产品介绍文章 | 高质量用户 |
| **V2EX** | 分享帖 | 技术用户 |
| **微博** | 产品安利 | 扩散 |

#### 冷启动策略
1. **种子用户**：自己 + 朋友圈 50 人内测
2. **内容营销**：写 3-5 篇"时间管理"相关文章，自然带产品
3. **社区运营**：在小红书/即刻持续分享使用心得
...
| 维度 | 规格 |
|---|---|
| 视觉 | **白橘花 + 小橘子混合**：14 朵白色橘花（参考真实柑橘花，主体）+ **≥ 6 个小橘子符号**（橘色锚点，硬约束保证品牌识别度），共 ~20 个 raindrops 横向均匀分布从屏幕顶部 2-98% 范围飘落 |
| 时长 | 每片飘落 4-5.8s；错峰下落（delay 0-2.5s）使整场约 6.5-8s |
| 分布算法 | **Jittered grid**（分桶 + 抖动）：横向 96% 宽度分 20 桶 + 时间 2.5s 分 20 段 → 保证不聚集、不一齐起降；blossoms / oranges 洗牌混排；大小 ±15% 自然变化 |
...
> ⚠️ 以下为基于公开数据的假设性分析，需实际用户访谈验证。

**待执行的用户研究**：
- 小红书"时间管理"话题下 50+ 篇帖子的内容分析
- 即刻"效率工具"圈子用户讨论分析
- 5-10 位目标用户（学生/职场新人）深度访谈
- 竞品 App Store 差评分析（Forest / 番茄 Todo / 滴答清单）

### 9.2 痛点假设验证

| 痛点假设 | 验证方式 | 当前置信度 |
|---------|---------|-----------|
| 用户有"计划失败感" | 小红书/即刻内容分析 | 高（大量相关内容） |
| 现有工具不解决"预期校准" | 竞品功能分析 | 高（无竞品做 record-first） |
| 用户愿意尝试新工具 | 访谈 | 中（需验证） |
| 双 aha 能提升留存 | 产品内测 | 未验证 |
...
| 风险类型 | 描述 | 概率 | 影响 | 缓解措施 |
|---------|------|------|------|---------|
| **技术** | localStorage 被清除导致数据丢失 | 中 | 高 | JSON 导出兜底 + 提示用户 |
| **技术** | 浏览器兼容性问题 | 低 | 中 | 只支持 Chrome/Safari/Edge |
| **市场** | 用户不愿从 Forest/Toggl 切换 | 高 | 高 | 差异化定位（record-first），不做同类竞品 |
| **市场** | 效率工具赛道拥挤 | 高 | 中 | Peel 定位独特，不在同一维度竞争 |
| **执行** | 一个人开发工期过长 | 中 | 中 | MVP 严格控范围，砍掉所有非核心功能 |
| **执行** | 设计质量达不到预期 | 中 | 高 | 用 Things 3 / Linear 做设计 anchor |
| **数据** | 用户量增长缓慢 | 高 | 中 | 先验证留存再追求增长 |
...
### v0.2.0 — 2026-05-27 · PRD 补全（市场/竞品/商业/风险）

#### 新增章节
- § 1.4 市场规模（TAM/SAM/SOM）— 全球 $8B 时间管理市场，中国 C 端 ~$500M
- § 1.5 竞品详细分析 — 5 竞品功能对比 + 定价对比 + 用户评价分析
- § 1.6 商业模式 — v1 免费验证，v1.1+ Freemium
- § 1.7 GTM 策略 — 小红书/即刻/少数派 + 冷启动方案
- § 9 用户研究 — 痛点假设验证 + 用户语言
- § 10 风险分析 — 7 项风险 + 缓解措施

#### 修改来源
- 触发：Obsidian 模板审查发现 PRD 模板缺关键章节
- 方法：AIPM 面试官视角评审 → 逐项补全
- 工具：Claude Code + Obsidian AIPM Wiki 自进化机制

#### 数据说明
- 市场规模数据为公开报告估算，需 Sensor Tower / Statista 实际验证
- 用户评价为基于公开信息的假设分析，需实际调研验证
- 标注 `⚠️` 的数据点待实际验证
