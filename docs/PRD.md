...
| 维度 | 规格 |
|---|---|
| 视觉 | **白橘花 + 小橘子混合**：14 朵白色橘花（参考真实柑橘花，主体）+ 4 个小橘子符号（橘色锚点），共 ~18 个 raindrops 横向均匀分布从屏幕顶部 2-98% 范围飘落 |
| 时长 | 每片飘落 4-5.8s；错峰下落（delay 0-2.5s）使整场约 6.5-8s |
| 分布算法 | **Jittered grid**（分桶 + 抖动）：横向 96% 宽度分 18 桶 + 时间 2.5s 分 18 段 → 保证不聚集、不一齐起降；blossoms / oranges 洗牌混排；大小 ±15% 自然变化 |
